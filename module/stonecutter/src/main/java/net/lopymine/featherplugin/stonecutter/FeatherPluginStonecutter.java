package net.lopymine.featherplugin.stonecutter;
...
import net.lopymine.featherplugin.stonecutter.tasks.*;
import org.gradle.*;
...
@ExtensionMethod(FeatherUtils.class)
public class FeatherPluginStonecutter implements Plugin<Project> {
	...
	public void apply(@NotNull Project project) {
		...
		for (StonecutterProject version : controller.getVersions()) {
			String loader = FeatherUtils.substringBefore(version.getProject(), "-");
	...
	public static List<String> getVersionsSpecifications(@NotNull Project project, String loader) {
		return Arrays.stream(FeatherUtils.getProperty(project, "%s.versions_specifications".formatted(loader))
				.split(" "))
				.map((version) -> FeatherUtils.substringBefore(version, "["))
				.toList();
	}
