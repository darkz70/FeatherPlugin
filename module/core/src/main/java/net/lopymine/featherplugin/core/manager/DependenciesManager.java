...
import net.lopymine.featherplugin.core.extension.*;
import net.lopymine.featherplugin.core.extension.FeatherCoreAdditionalDependencies.AdditionalDependencyOverride;
...
public class DependenciesManager {
	...
	private static void addCustomConfigurations(@NotNull FeatherProjectConfigurationData data) {
	...
	private static void addDependencies(@NotNull FeatherProjectConfigurationData data, FeatherCoreDependenciesExtension extension) {
		Project project = data.project();
		LoaderManager loaderManager = data.loaderManager();
		...
		Map<String, String> properties = project.getFeatherProperties("dep");
		FeatherCoreAdditionalDependencies additional = extension.getAdditional();
		...
		if (yaclVersion != null && !yaclVersion.equals("unknown")) {
			...
			if (oldMavenVersions.contains(minecraft)) {
				dependencies.add(configurationName, "dev.isxander.yacl:yet-another-config-lib-fabric:%s".formatted(FeatherUtils.substringBeforeLast(yaclVersion, "-")));
			} else {
