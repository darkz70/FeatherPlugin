...
import net.lopymine.featherplugin.core.util.MultiVersion;
...
public class ModPublishManager {
	...
	public static void apply(@NotNull FeatherProjectConfigurationData data, ModPublishExtension mpe) {
		...
		FeatherPluginCore.LOGGER.logModule("MPP","Configuring \"%s\"", mpe.getDisplayName().get());
		FeatherPluginCore.LOGGER.logModule("MPP","Dry Run: %s", mpe.getDryRun().get());
	}

	private static Provider<RegularFile> getModFile(FeatherProjectConfigurationData data) {
