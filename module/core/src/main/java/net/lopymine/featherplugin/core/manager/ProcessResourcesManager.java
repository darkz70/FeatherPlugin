...
import net.lopymine.featherplugin.core.extension.FeatherCoreProcessResourcesExtension;
...
public class ProcessResourcesManager {
	...
	public static void apply(@NotNull FeatherProjectConfigurationData data) {
		...
		project.getExtensions().create("mossyResources", FeatherCoreProcessResourcesExtension.class);
		...
		project.getGradle().addProjectEvaluationListener(new ProjectEvaluationListener() {
			...
			public void afterEvaluate(@NotNull Project project, @NotNull ProjectState state) {
				FeatherCoreProcessResourcesExtension extension = project.getExtensions().getByType(FeatherCoreProcessResourcesExtension.class);
	...
	private static void processResources(@NotNull FeatherProjectConfigurationData data, FeatherCoreProcessResourcesExtension extension) {
		Project project = data.project();
		...
		Map<String, String> properties = project.getFeatherProperties("data");
		properties.putAll(project.getFeatherProperties("build"));
		properties.putAll(project.getFeatherProperties("dep"));
		...
		if (i != -1 && i != data.minecraftVersion().lastIndexOf("-")) {
			properties.put("minecraft", "%s-%s".formatted(data.comparableMinecraftVersion(), FeatherUtils.substringSince(data.minecraftVersion(), "-").replace("-", ".")));
		} else {
