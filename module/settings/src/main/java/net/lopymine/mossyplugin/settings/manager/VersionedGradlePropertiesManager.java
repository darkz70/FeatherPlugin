...
public class VersionedGradlePropertiesManager {
	...
	public static void createGradleProperties(Path rootPath, String version, List<String> additionalDependencies, Properties rootGradleProperties, BiFunction<String, String, String> dependResolver, Function<String, String> yarnResolver) throws IOException {
		...
		if (isEmpty) {
			...
			FeatherPluginSettings.LOGGER.log("Successfully created gradle.properties for " + version);
			return;
		}
		...
		if (update) {
			...
			FeatherPluginSettings.LOGGER.log("Successfully updated gradle.properties for " + version);
		}
		...
		if (!missingDependencies.isEmpty()) {
			...
			Files.writeString(file.toPath(), builder.toString(), StandardCharsets.UTF_8);
			FeatherPluginSettings.LOGGER.log("Successfully added new depends " + missingDependencies + " to gradle.properties for " + version);
		}
		...
		if (!oldDependencies.isEmpty()) {
			...
			FeatherPluginSettings.LOGGER.log("Successfully removed old depends " + removed + " from gradle.properties for " + version);
		}
	}
