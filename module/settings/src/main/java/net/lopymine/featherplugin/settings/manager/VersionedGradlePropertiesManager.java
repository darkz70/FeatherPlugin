...
import net.lopymine.featherplugin.settings.api.*;
import net.lopymine.featherplugin.settings.loader.*;
...
public class VersionedGradlePropertiesManager {
	...
	public static void apply(@NotNull Settings settings, Properties gradleProperties, List<FeatherProject> projects, List<String> additionalDependencies) {
		...
		for (FeatherProject project : projects) {
			try {
				...
				FeatherPluginSettings.LOGGER.log("Failed to create versioned gradle properties for " + project.projectName() + ", reason: " + e.getMessage(), e);
	...
	public static void createGradleProperties(
		...
		for (String key : gradleProperties.stringPropertyNames()) {
			...
			String dependencyId = FeatherUtils.substringSince(key, ".");
	...
	private static File getOrCreateGradlePropertiesFile(Path path, String version) {
		try {
			...
			FeatherPluginSettings.LOGGER.log("Failed to create gradle.properties file!");
			e.printStackTrace(System.out);
			return null;
		}
	}
