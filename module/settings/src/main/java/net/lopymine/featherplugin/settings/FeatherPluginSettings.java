package net.lopymine.featherplugin.settings;

import java.io.*;
import java.util.*;
import lombok.Getter;
import net.lopymine.featherplugin.common.*;
import net.lopymine.featherplugin.settings.loader.LoaderManager;
import net.lopymine.featherplugin.settings.manager.*;
import net.lopymine.featherplugin.settings.project.FeatherProject;
...
public class FeatherPluginSettings implements Plugin<Settings> {
	...
	public void apply(@NotNull Settings settings) {
		...
		settings.getRootProject().setName(FeatherUtils.getProperty(gradleProperties, "data.mod_name"));
		...
		List<FeatherProject> projects = getFeatherProjects(loadersAndVersions);
	...
	public static Map<String, List<String>> getLoadersAndVersions(Properties gradleProperties, List<String> loaders) {
		...
		for (String loader : loaders) {
			List<String> versions = Arrays.stream(FeatherUtils.getProperty(gradleProperties, "%s.multi_versions".formatted(loader)).split(" ")).toList();
			map.put(loader, versions);
		}
		return map;
	}

	public static List<FeatherProject> getFeatherProjects(Map<String, List<String>> loaderAndVersions) {
		List<FeatherProject> projects = new ArrayList<>();

		loaderAndVersions.forEach((loader, versions) -> {
			LoaderManager loaderManager = LoaderManager.of(loader);
			for (String version : versions) {
				projects.add(new FeatherProject("%s-%s".formatted(loader, version), loader, version, FeatherUtils.substringBefore(version, "-"), loaderManager));
			}
		});
	...
	public static List<String> getLoaders(Properties gradleProperties) {
		return Arrays.stream(FeatherUtils.getProperty(gradleProperties, "mod_loaders").split(" ")).toList();
	}
	...
	public static @NotNull Properties getGradleProperties(File project) {
		...
		try (FileReader reader = new FileReader(project.toPath().resolve("gradle.properties").toFile())) {
			...
			FeatherPluginSettings.LOGGER.log("Failed to read gradle.properties from \"%s\"!".formatted(project.getAbsolutePath()));
