package net.darkz70.featherplugin.settings.api;
...
public class ModrinthDependenciesAPI {
	...
	public static String getVersion(String modId, String minecraftVersion, String loader) {
		...
		try {
			...
			FeatherPluginSettings.LOGGER.log("Failed to find Modrinth project with id \"%s\"", modId);
			e.printStackTrace(System.out);
			return "unknown";
		} catch (Exception e) {
			FeatherPluginSettings.LOGGER.log("Failed to get version from Modrinth project with id \"%s\"", modId);
			e.printStackTrace(System.out);
			return "unknown";
		}
